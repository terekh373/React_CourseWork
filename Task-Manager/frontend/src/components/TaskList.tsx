...
const TaskList: React.FC<TaskListProps> = ({ token }) => {
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      ...
      try {
        ...
        setCurrentUser(res.data);           // { username, displayName, avatar }
        // Важно: в ответе есть _id ? Если нет — смотри ниже
      } catch (err) {
        console.error('Не удалось загрузить данные пользователя', err);
        // можно показать toast или просто продолжить без фильтра "Мои"
      } finally {
        setLoadingUser(false);
      }
    };
  ...
  const fetchStats = async () => {
    try {
      const allTasksRes = await axios.get<Task[]>('http://localhost:5000/api/tasks', {
        headers: { Authorization: `Bearer ${token}` },
      });
      const archiveRes = await axios.get<Task[]>('http://localhost:5000/api/tasks/archive', {
        headers: { Authorization: `Bearer ${token}` },
      });

      const allTasks: Task[] = [...allTasksRes.data, ...archiveRes.data];

      const active = allTasksRes.data.length;
      const done = allTasks.filter((t: Task) => t.status === 'Done' && !t.isArchived).length;
      const overdue = allTasks.filter((t: Task) =>
        t.deadline && new Date(t.deadline) < new Date() && t.status !== 'Done' && !t.isArchived
      ).length;
      const archived = archiveRes.data.length;

      // По статусам (только активные)
      const byStatus = {
        'To Do': allTasksRes.data.filter((t: Task) => t.status === 'To Do').length,
        'In Progress': allTasksRes.data.filter((t: Task) => t.status === 'In Progress').length,
        'Done': done,
      };

      // По приоритетам
      const byPriority = {
        Low: allTasksRes.data.filter((t: Task) => t.priority === 'Low').length,
        Medium: allTasksRes.data.filter((t: Task) => t.priority === 'Medium').length,
        High: allTasksRes.data.filter((t: Task) => t.priority === 'High').length,
      };

      // По меткам (топ-5)
      const labelCount: Record<string, number> = {};
      allTasksRes.data.forEach((t: Task) => {
        t.labels?.forEach((l: Label) => {
      ...
      // 5 самых старых просроченных
      const recentOverdue = allTasks
        .filter((t: Task) => t.deadline && new Date(t.deadline) < new Date() && t.status !== 'Done' && !t.isArchived)
  ...
  return (
    <div className={`min-vh-100 ${darkMode ? 'bg-dark text-light' : 'bg-light text-dark'}`}>
        ...
        <Row className="align-items-center mb-4">
          <Col>
            <div className="d-flex align-items-center gap-3">
              ...
              <h1 className="fw-bold mb-0">{currentDisplayName || 'Користувач'}</h1>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Архив задач</Modal.Title>
        </Modal.Header>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Статистика задач</Modal.Title>
        </Modal.Header>
        <Modal.Body>
