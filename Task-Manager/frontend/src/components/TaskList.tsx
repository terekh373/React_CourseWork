...
const TaskList: React.FC<TaskListProps> = ({ token }) => {
  ...
  const removeFriend = async (friendId: string) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/friends/${friendId}/remove`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <div style={{
              ...
              <div className="d-flex align-items-center">
                {activeBoard.members.slice(0, 5).map((m: UserInfo, i: number) => (
                  <div key={m._id} style={{ marginLeft: i === 0 ? 0 : -8, zIndex: 5 - i, position: 'relative' }} title={m.displayName}>
                    {m.avatar ? (
                      <Image src={`http://localhost:5000${m.avatar}`} roundedCircle width={28} height={28} style={{ border: '2px solid rgba(255,255,255,0.5)', objectFit: 'cover' }} />
                    ) : (
          ...
          <div className="d-flex align-items-center gap-2 flex-wrap">
            ...
            <Dropdown align="end">
              <Dropdown.Toggle
                ...
                {avatar ? (
                  <Image src={`http://localhost:5000${avatar}`} roundedCircle width={28} height={28} style={{ objectFit: 'cover', border: '2px solid rgba(255,255,255,0.5)' }} />
                ) : (
                                          ...
                                          <div className="d-flex align-items-center justify-content-between" style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '0.45rem', marginTop: '0.25rem' }}>
                                            ...
                                            <div className="d-flex align-items-center gap-1">
                                              {task.createdBy?.avatar ? (
                                                <Image src={`http://localhost:5000${task.createdBy.avatar}`} roundedCircle width={20} height={20} style={{ objectFit: 'cover', border: '1.5px solid rgba(255,255,255,0.35)', flexShrink: 0 }} />
                                              ) : (
      ...
      <Modal show={showProfileModal} onHide={() => setShowProfileModal(false)} centered>
        ...
        <Modal.Body style={{ ...glassCard, borderRadius: 0, textAlign: 'center', paddingTop: '1.5rem', paddingBottom: '1.5rem' }}>
          <div className="mb-3">
            {avatar ? (
              <Image src={`http://localhost:5000${avatar}`} roundedCircle width={100} height={100} style={{ objectFit: 'cover', border: '3px solid rgba(255,255,255,0.4)' }} />
            ) : (
      ...
      <Modal show={showNewBoardModal} onHide={() => setShowNewBoardModal(false)} centered>
        ...
        <Modal.Footer style={glassCard}>
          ...
          <Button className="reglog-btn" disabled={!newBoardName.trim()} style={{ border: 'none' }}
            onClick={async () => {
              try {
                await axios.post('http://localhost:5000/api/boards', { name: newBoardName.trim() }, { headers: { Authorization: `Bearer ${token}` } });
                const boardsRes = await axios.get<Board[]>('http://localhost:5000/api/boards', { headers: { Authorization: `Bearer ${token}` } });
      ...
      <Modal show={showJoinModal} onHide={() => setShowJoinModal(false)} centered>
        ...
        <Modal.Footer style={glassCard}>
          ...
          <Button className="reglog-btn" disabled={!joinCodeInput.trim()} style={{ border: 'none' }}
            onClick={async () => {
              try {
                const res = await axios.post('http://localhost:5000/api/boards/join', { inviteCode: joinCodeInput.trim() }, { headers: { Authorization: `Bearer ${token}` } });
        ...
        <Modal.Body style={{ ...glassCard, borderRadius: 0 }}>
            ...
            : friendsData.friends.map(f => (
              <div key={f._id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.6rem 0', borderBottom:'1px solid rgba(255,255,255,0.08)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  {f.avatar
                    ? <Image src={`http://localhost:5000${f.avatar}`} roundedCircle width={42} height={42} style={{ objectFit:'cover' }} />
            ...
            : friendsData.incoming.map(req => (
              <div key={req.requestId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.6rem 0', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  {req.from?.avatar && <Image src={`http://localhost:5000${req.from.avatar}`} roundedCircle width={38} height={38} />}
          ...
          {activeFriendsTab === 'outgoing' && (friendsData.outgoing.length === 0
            ...
            : friendsData.outgoing.map(req => (
              <div key={req.requestId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.6rem 0', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  {req.to?.avatar && <Image src={`http://localhost:5000${req.to.avatar}`} roundedCircle width={38} height={38} />}
