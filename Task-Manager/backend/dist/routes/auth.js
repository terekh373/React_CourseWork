...
router.put('/change-displayname', authMiddleware, async (req, res) => {
    ...
    try {
        const user = await User.findByIdAndUpdate(req.userId, { displayName: newDisplayName }, { new: true }).select('-password');
...
router.put('/change-username', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const user = await User.findByIdAndUpdate(req.userId, { username: newUsername }, { new: true }).select('-password');
