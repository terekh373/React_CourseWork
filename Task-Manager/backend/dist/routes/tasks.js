...
router.post('/', async (req, res) => {
    if (!req.body.board)
        return res.status(400).json({ message: 'board required' });
    try {
        const task = new Task({
            ...req.body,
...
router.put('/:id', async (req, res) => {
    try {
        const task = await Task.findOneAndUpdate({ _id: req.params.id }, req.body, { new: true }).populate('createdBy', 'displayName avatar username');
        if (!task)
...
router.delete('/:id', async (req, res) => {
    try {
        const task = await Task.findOneAndUpdate({ _id: req.params.id }, // 🔥 убрали user
        { isArchived: true, archivedAt: new Date() }, { new: true });
        if (!task)
            return res.status(404).json({ message: 'Task not found' });
...
router.post('/:id/restore', async (req, res) => {
    try {
        const task = await Task.findOneAndUpdate({ _id: req.params.id }, { isArchived: false, archivedAt: null }, { new: true }).populate('createdBy', 'displayName avatar username');
        if (!task)
            return res.status(404).json({ message: 'Task not found' });
